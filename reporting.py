...
class ReportManager:
    ...
    def generate_report_tag_expenses(self):
        ...
        try:
            ...
            # Create only a horizontal bar chart for counts (removed pie chart)
            if results:
                ...
                sorted_data = sorted(zip(tags, counts), key=lambda x: x[1])
                tags = [x[0] for x in sorted_data]
                counts = [x[1] for x in sorted_data]
                
                # Bar chart for usage count
                bars = plt.barh(tags, counts, color='teal')
